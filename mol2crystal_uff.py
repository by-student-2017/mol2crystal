...
def uff_optimize(fname):
    try:
        ...
        # Read and optimize with Open Babel
        mol = next(pybel.readfile("xyz", temp_xyz))
        mol.make3D()
        mol.localopt(forcefield="uff", steps=10000)
        ...
        mol.write("vasp", opt_fname)
        print(f"[UFF] Saved: {opt_fname}")
        ...
        import traceback
        traceback.print_exc()
        print(f"Error optimizing {fname} with UFF: {e}")
